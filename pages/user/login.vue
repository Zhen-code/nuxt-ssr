...
export default {
  ...
  asyncData({ $axios, app: { $api }}) {
    // console.log(context.app.$request.testPost)
    // $axios.
    // axiso.get('https://api.virapi.com/vir_gitee207bge4fed2dc/demo/menu', {
    //   headers: {
    //     'Content-Type': 'application/json',
    //     'app-token': '$2a$10$e8B6ynJm9JR4.n3ITfJNtujGCbWwiouqCRNKhjBArXdbqqL03MOMS'
    //   }
    // }).then((res) => {
    //   console.log(res)
    // }).catch((err) => {
  ...
  beforeCreate() {
    // console.log(this.$request.testGet, '++')
  ...
  methods: {
    ...
    handleLogin() {
      this.$refs.loginForm.validate((valid) => {
        if (valid) {
          this.loading = true
          postRequest('http://192.168.0.124:8087/login', {
            username: 'sys2',
